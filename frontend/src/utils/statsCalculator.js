...
export function calculateBatterStats(records) {
  const batterRecords = records.filter(
    (r) => r.gameType === "Batter" || r.gameType === "Both"
  );
  ...
  const totalStats = batterRecords.reduce(
    (acc, record) => {
  ...
  const battingAvg =
    totalStats.totalAB > 0
      ? (totalStats.totalH / totalStats.totalAB).toFixed(3)
      : "0.000";
  ...
  const obp =
    obpDenominator > 0
      ? (totalStats.totalH + totalStats.totalBB + totalStats.totalHBP) /
        obpDenominator
      : 0;

  const slg =
    totalStats.totalAB > 0 ? totalStats.totalBases / totalStats.totalAB : 0;
...
export function calculatePitcherStats(records) {
  const pitcherRecords = records.filter(
    (r) => r.gameType === "Pitcher" || r.gameType === "Both"
  );
  ...
  const totalStats = pitcherRecords.reduce(
    (acc, record) => {
